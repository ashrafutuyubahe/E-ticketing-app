...
import driverLogin from './routes/Driverauth';
import ticketScheduleUpload from './routes/ticketscheduleUpload';
import TicketSchedule from './routes/ticketSchedule';
import TicketScheduleModel from './models/scheduleModel';
import Driver from './models/driverModel';
import authenticateToken from './middlewares/userAuth';
import Ticket from './models/ticketsModel';
import BoughtTicket from './models/boughtTicketModel';
import wholeUserAuth from './routes/wholeuseAuth';
import findAvailableSeats from './routes/ticketSchedule';
import AdminAuthentication from './routes/adminauth';
import DownloadReport from './routes/downloadReport';
import AgentAuthentication from './routes/agentauthentication';
import User from './models/users';
import sendSMS from './msgconfig';
...
app.post('/AddDrivers', async (req, res) => {
  ...
  try {
    ...
    if (existingDriver) {
      return res
        .status(400)
        .json({ error: 'Driver with that name and car already exists' });
    }
    ...
    return res
      .status(201)
      .json({ message: 'Driver has been added successfully' });
  } catch (err) {
    console.error(err);
...
app.post('/addTickets', async (req, res) => {
  ...
  try {
    ...
    if (findTicketsExists) {
      return res
        .status(401)
        .send(' there is a similar ticket please add different ticket');
    }
...
app.post('/getYourBoughtTicket', async (req, res) => {
  ...
  try {
    const schedule = await TicketScheduleModel.findOne({
      carPlate: vehicleNumber,
    });

    if (!schedule) {
      return res.status(404).json({
        error: 'No driver found for the provided vehicle number',
      });
    }
...
app.post('/scanTicket', async (req, res) => {
  try {
    ...
    if (ticket.paymentStatus !== paymentStatus) {
      return res
        .status(400)
        .json({ error: 'Ticket payment status does not match' });
    }
