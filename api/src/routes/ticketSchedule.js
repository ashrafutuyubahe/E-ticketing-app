...
const log = debugModule('ticketSchedule');
...
const updateSeatAvailability = async (carPlate) => {
  try {
    ...
    const availableSeats = TOTAL_SEATS - ticketCount;

    const message = JSON.stringify({
      type: 'update',
      carPlate,
      availableSeats,
      isSoldOut: ticketCount >= TOTAL_SEATS,
    });
    ...
    log('Error updating seat availability:', error);
  }
};
  ...
  try {
    ...
    if (schedules.length === 0) {
      return res.status(404).json({
        message: 'No ticket schedules found for the specified route and agency',
      });
    }
  ...
  try {
    ...
    boughtTickets.forEach((ticket) => {
      ...
      if (groupedSchedules[route]) {
        ...
        groupedSchedules[route].availableSeats = Math.max(
          0,
          groupedSchedules[route].availableSeats - seatsPurchased
        );
...
Router.put('/updateSchedule/:id', async (req, res) => {
  try {
    ...
    await Ticket.findByIdAndUpdate(updateTicketExists._id, updatedTicket, {
      new: true,
    });
